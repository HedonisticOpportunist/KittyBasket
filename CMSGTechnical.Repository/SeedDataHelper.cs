...
// Helper class for seeding initial data into the EF Core model.
internal static class SeedDataHelper
{
    // Entry point for all seed operations.
    // Extends ModelBuilder so it can be called fluently in OnModelCreating.
    ...
    // Seed a default basket so the application always has a starting basket record.
    private static void SeedBasket(this ModelBuilder builder)
    {
        builder.Entity<Basket>().HasData(new Basket { Id = 1 });
    }

    // Seed the menu with a predefined list of items.
    // Uses anonymous objects because EF Core requires concrete values for HasData.
